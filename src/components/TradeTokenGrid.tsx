...
export class TradeTokenGrid extends Component<ITradeTokenGridProps, ITradeTokenGridState> {
  ...
  constructor(props: ITradeTokenGridProps) {
    ...
    TradeTokenGrid.defaultUnitOfAccount = process.env.REACT_APP_ETH_NETWORK === "kovan" ? Asset.SAI : Asset.DAI;
  ...
  public componentDidMount(): void {
    ...
    this.props.onSelect(
      new TradeTokenKey(e.asset, e.defaultUnitOfAccount, e.positionType, e.defaultLeverage, e.defaultTokenizeNeeded)
    );
  ...
  public componentDidUpdate(
    prevProps: Readonly<ITradeTokenGridProps>,
    prevState: Readonly<ITradeTokenGridState>,
    snapshot?: any
  ): void {
  ...
  private renderDesktop = () => {
    ...
    if (this.props.selectedKey.asset !== Asset.UNKNOWN) {
      tokenRows = this.state.tokenRowsData
        .filter(e => e.positionType == (this.props.isLong ? PositionType.LONG : PositionType.SHORT))
        .map(e => <TradeTokenGridRow isMobileMedia={false} key={`${e.asset}_${e.positionType}`} {...e} />);
    }
      ...
      <div className="trade-token-grid__wrapper">
        <div className="trade-token-grid">
          <TradeTokenGridHeader
            asset={this.props.selectedKey.asset}
            showMyTokensOnly={this.props.showMyTokensOnly}
			isMobileMedia={false}
			positionType={this.props.isLong ? PositionType.LONG : PositionType.SHORT}
			onPositionTypeChange={(value) => this.props.changeActiveBtn(value)}
          />
          {tokenRows &&
            tokenRows.map(row => {
              return (
                <div className="trade-token-grid-row-wrapper" key={`${row.props.asset}_${row.props.positionType}`}>
                  {row}
                  <OwnTokenGrid
                    positionType={row.props.positionType}
                    asset={row.props.asset}
                    {...tradeTokenGridProps}
                  />
                </div>
              );
  ...
  private renderMobile = () => {
    const tokenRowsMobile = this.state.tokenLongShortRowsData.map(e => (
      <TradeTokenCardMobile
        key={`${e.asset}_${e.positionType}`}
        {...e}
        showMyTokensOnly={this.props.showMyTokensOnly}
      />
    ));
    console.log("asd");
    return <div className="trade-token-card-mobile__wrapper">{tokenRowsMobile}</div>;
  };
